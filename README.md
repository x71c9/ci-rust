...
Updates four package repositories in parallel after a release. Every package file is regenerated from the template on each release, so hand edits in the package repositories are overwritten.

- **AUR** (`<name>` source package + `<name>-bin` binary package) — creates the AUR repo on first publish if it doesn't exist yet.
- **NUR** (`x71c9/nur-packages`) — scaffolds `pkgs/<name>/default.nix` on first publish, registers the package in the root `default.nix`, and rewrites `default.nix`, source `hash`, `Cargo.lock` and `README.md` on every release.
- **Homebrew** (`x71c9/homebrew-x71c9`) — rewrites `Formula/<name>.rb` on every release.

When `has_shell_completions` is set, every target installs bash, zsh and fish completions generated by running `<binary> <completion_subcommand> <shell>` at package build time. When `bin_aliases` is set, every target installs the listed names as symlinks to the binary and installs a completion file for each alias too (bash-completion and fish load completions by command name).
...
A project needs a `ci.toml` at the repository root plus two thin caller files in `.github/workflows/`. The callers pass only the repository name; all package configuration is read from `ci.toml` at release time.

### `ci.toml`

```toml
# Required
crate_name  = "my-crate"   # name in Cargo.toml (may differ from the binary)
binary_name = "my-tool"    # actual binary name; also the AUR/NUR/Homebrew package name
pkgdesc     = "Short description of the tool"

# Optional
# aur_depends           = "'glibc'"        # AUR depends=(); default 'glibc'
# nur_build_inputs      = ""               # nixpkgs attrs for buildInputs, e.g. "xorg.libxcb"
# nur_description       = ""               # NUR meta.description; defaults to pkgdesc
# has_shell_completions = "true"           # binary exposes `<binary> <completion_subcommand> <shell>`
# completion_subcommand = "completion"     # default
# bin_aliases           = "t tmps"         # extra command names installed as symlinks to the binary
# extra_apt_packages    = ""               # apt packages for the Ubuntu build runners
# macos_extra_brew      = ""               # e.g. "--cask xquartz"
# macos_extra_env       = ""               # e.g. "LIBRARY_PATH=/opt/X11/lib PKG_CONFIG_PATH=/opt/X11/lib/pkgconfig"
```

Values are plain double-quoted strings, one key per line. Keys are matched by `^key =`, so a commented-out key is ignored.
